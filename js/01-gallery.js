...
function pictureClick(event) { // обработчик события клика на элемент галереи
  if (event.target.nodeName !== 'IMG') { // проверка на клик на элементе галереи
    return; // если клик не по изображению, то выходим из функции
  }
  const instance = basicLightbox.create( // создание модального окна
